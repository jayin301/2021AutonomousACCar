...
def gradient_combine(img, th_x, th_y, th_mag, th_dir):
    ...
    # cv2.imshow('mag',mag_img)

    gradient_comb = np.zeros_like(R).astype(np.uint8)
    gradient_comb[((sobelx > 1) &(mag_img > 1))| ((mag_img > 1) & (sobely > 1))] = 255
...
def hls_combine(img, th_h, th_l, th_s):
    ...
    H = hls[200:rows - 140, 0:cols, 0]
    L = hls[200:rows - 140, 0:cols, 1]
    S = hls[200:rows - 140, 0:cols, 2]
    ...
    hls_comb[(s_img > 1) & (l_img == 0) & (h_img > 1)] = 255
    return hls_comb
